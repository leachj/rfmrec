...
#include <PinChangeInterruptTiny.h>
...
#define SW_PIN 0         // Reed switch connected from ground to this pin (D0/ATtiny pin 13)
#define LDR_POWER 7
...
void setup() {
  ...
  attachPcInterrupt(SW_PIN,wakeUp,CHANGE); // attach a PinChange Interrupt on the falling edge
...
void loop() {
  ...
  tinytx.pir = (switchState == LOW)?1:0;

  bitClear(PRR, PRADC); ADCSRA |= bit(ADEN); // Enable the ADC
  tinytx.light = 1024-analogRead(2);   // read the input pin (A2)
  ADCSRA &= ~ bit(ADEN); bitSet(PRR, PRADC); // Disable the ADC to save power
